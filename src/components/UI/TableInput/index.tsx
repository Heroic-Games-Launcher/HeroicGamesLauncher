  ...
  return (
    <>
      <table>
        <tbody>
        <tr>
          {!!header.length && header.map((entry: string, key) => {
            return <th key={key}>{entry}</th>
          })}
          <th></th>
        </tr>
        {!!rowData.length && rowData.map((row: TableInputRow, key) => {
          return (
            <tr key={key}>
              {!!row.values.length && row.values.map((value: string, key) => {
