...
async function addShortcuts(game: Game, fromMenu?: boolean) {
  ...
  logInfo(`Adding shortcuts for ${game}`, LogPrefix.Backend)
  ...
  const launchWithProtocol = `heroic://launch?appName=${game.id}&runner=${game.runner}`
...
`
    ...
    case 'win32': {
      ...
      if (gameInfo.runner === 'gog') {
        executable = libraryManagerMap['gog'].getExecutable(game.id)
      }
      if (executable) {
        let icon: string
        if (
...
async function generateMacOsApp(game: Game) {
  const gameInfo = game.getGameInfo()
  ...
  const isIconIcns = await convertPngToICNS(game, resourcesFolder)
  ...
  `
    ...
    const launchCommand = `${app.getPath(
      'exe'
    )} --no-gui heroic://launch?appName=${game.id}&runner=${game.runner}`
...
async function convertPngToICNS(game: Game, dest: string) {
  try {
    const iconPath = await getIcon(game)
