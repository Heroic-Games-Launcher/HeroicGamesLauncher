import { mkdirSync, rmSync, symlinkSync, writeFileSync } from 'graceful-fs'
...
describe('getLinuxWineSet - Proton detection', () => {
  ...
  beforeEach(() => {
    ...
    rmSync(testToolsPath, { force: true, recursive: true })
      ...
      const vdfContent = `"compatibilitytools"
{
  ...
  describe('Non-VDF fallback detection', () => {
    ...
    it('should detect proton from symlinked directory structure without VDF', async () => {
      const actualProtonDir = join(tmpdir(), 'heroic-test-symlink-target')
      const symlinkedProtonDir = join(protonPath, 'SymlinkProton')
      const protonBin = join(symlinkedProtonDir, 'proton')

      rmSync(actualProtonDir, { force: true, recursive: true })
      mkdirSync(actualProtonDir, { recursive: true })
      writeFileSync(
        join(actualProtonDir, 'proton'),
        '#!/bin/bash\necho "proton"'
      )
      symlinkSync(actualProtonDir, symlinkedProtonDir, 'dir')
      ...
      const symlinkProton = protonInstalls.find(
        (p) => p.name === 'SymlinkProton'
      )
      expect(symlinkProton).toBeDefined()
      expect(symlinkProton?.bin).toBe(protonBin)
    })

    it('should fall back to proton binary when directory VDF is invalid', async () => {
      const protonDir = join(protonPath, 'InvalidManifestProton')
      const protonBin = join(protonDir, 'proton')
      mkdirSync(protonDir, { recursive: true })
      writeFileSync(protonBin, '#!/bin/bash\necho "proton"')
      writeFileSync(join(protonDir, 'compatibilitytool.vdf'), 'not valid vdf')
      ...
      const invalidManifestProton = protonInstalls.find(
        (p) => p.name === 'InvalidManifestProton'
      )
      expect(invalidManifestProton).toBeDefined()
      expect(invalidManifestProton?.bin).toBe(protonBin)
    })
