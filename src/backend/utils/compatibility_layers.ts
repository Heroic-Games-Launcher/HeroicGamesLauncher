...
function scanProtonPath(path: string): Set<WineInstallation> {
  ...
  try {
    ...
    // Track which directories were successfully detected via VDF
    const processedDirs = new Set(
      vdfInstallations
        .map((installation) => relative(path, dirname(installation.bin)))
        .filter(
          (itemDir) =>
            itemDir &&
            itemDir !== '..' &&
            !itemDir.startsWith(`..${sep}`) &&
            !isAbsolute(itemDir)
        )
    )
    ...
    const protonInstallations = readdirSync(path)
      .map((itemName) => ({
        protonBin: join(path, itemName, 'proton'),
        itemDir: itemName,
        itemName
      }))
      .filter(({ itemName }) => !itemName.startsWith('UMU-Latest'))
      .filter(({ protonBin }) => existsSync(protonBin))
