...
import { installVersion } from '../../toolmanager'
import { VersionInfo } from '../../../../common/types/toolmanager'
...
describe('ToolManager - InstallVersion', () => {
  ...
  test('install fails because installDir is not a directory', async () => {
    ...
    await installVersion({
      ...
      .catch((error) => {
        expect(error.message).toBe(
          `Installation directory ${workDir}/src/backend/toolmanager/__test__/toolmanager/install.test.ts is not a directory!`
        )
      })
  })
