...
describe('Utilities - Downlaod', () => {
  ...
  test('download file fails because of installDir is a file', async () => {
    ...
    await downloadFile({
      ...
      .catch((error) => {
        expect(error).toBe(
          `Download path ${workDir}/src/backend/toolmanager/__test__/utils/download.test.ts is not a directory!`
        )
      })
  })
    ...
    await downloadFile({
      ...
      .then((response) => {
        expect(response).toBe(
          `Succesfully downloaded file:///${workDir}/src/backend/toolmanager/__test__/utils/../test_data/test.tar.xz to ${workDir}/src/backend/toolmanager/__test__/utils/test_download/test.tar.xz.`
        )
      })
      .catch(() => {
        failed = true
