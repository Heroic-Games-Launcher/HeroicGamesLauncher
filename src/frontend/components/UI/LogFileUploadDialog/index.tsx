...
export default function LogUploadDialog() {
  ...
  const [dialogContent, dialogFooter] = useMemo(() => {
    if (!uploadLogFileProps) return ['', '']
    if (!confirmed)
      return [
        t(
          'setting.log.upload.confirm',
    ...
    if (uploading)
      return [
        <>
          <CircularProgress />
          {t('setting.log.upload.uploading', 'Uploading log file...')}
        </>,
        <></>
      ]
    if (error)
    ...
    return [
      t(
        'setting.log.upload.done',
        'Upload complete. Uploaded to {{url}} (URL copied to your clipboard)',
        {
          url: uploadUrl
        }
      ),
