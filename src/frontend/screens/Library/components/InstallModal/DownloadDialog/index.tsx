...
export default function DownloadDialog({
  ...
  const [installPath, setInstallPath] = useState(gameStatus.folder || 'default')
  ...
  const downloadSize = () => {
    if (gameInstallInfo?.manifest?.download_size) {
      if (gameStatus.folder === installPath) {
        const progress = 100 - (gameStatus?.progress?.percent ?? 0)
        return size(
  ...
  function getInstallLabel() {
    if (installPath) {
      if (notEnoughDiskSpace) {
        ...
        return gameStatus.folder === installPath
