...
describe.skip('GeneralSettings', () => {
  ...
  test('change default install path', async () => {
    ...
    await waitFor(() =>
      expect(onSetDefaultInstallPath).toBeCalledWith("'another/install/path'")
    )
  ...
  test('change epic sync path', async () => {
    ...
    await waitFor(() =>
      expect(onSetEgsPath).toBeCalledWith("'another/sync/path'")
    )
  ...
  test('change epic sync path not working if linked', async () => {
    ...
    fireEvent.click(syncPathButton)
    await waitFor(() =>
      expect(onSetEgsPath).not.toBeCalledWith('another/sync/path')
    )
  ...
  test('sync fails with epic path', async () => {
    ...
    fireEvent.click(syncButton)
    await waitFor(() =>
      expect(ipcRenderer.invoke).toBeCalledWith('showErrorBox', [
        'box.error.title',
        'box.sync.error'
      ])
    )
  ...
  test('start minimized is hidden when exitToTray is false', async () => {
    ...
    expect(() => getByTestId('startInTray')).toThrow(
