...
const launchGame = (appName) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    return execAsync(command)
        .then(({ stderr }) => {
        fs_1.writeFile(`${heroicGamesConfigPath}${appName}-lastPlay.log`, stderr, () => 'done');
        if (stderr.includes('Errno')) {
            showErrorBox('Something Went Wrong', 'Error when launching the game, check the logs!');
        }
    })
...
const writeGameconfig = (game) => {
    const { wineVersion, winePrefix, otherOptions, useGameMode, showFps, } = JSON.parse(fs_1.readFileSync(heroicConfigPath)).defaultSettings;
    // @ts-ignore
    const { account_id } = JSON.parse(fs_1.readFileSync(userInfo));
    const config = {
        [game]: {
    ...
    if (!fs_1.existsSync(`${heroicGamesConfigPath}${game}.json`)) {
        fs_1.writeFileSync(`${heroicGamesConfigPath}${game}.json`, JSON.stringify(config, null, 2), 
        //@ts-ignore
        () => 'done');
    }
};
...
function getLatestDxvk() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (fs_1.existsSync(pastVersionCheck)) {
            pastVersion = fs_1.readFileSync(pastVersionCheck).toString().split('\n')[0];
        }
        if (pastVersion === name) {
            return;
        }
...
function installDxvk(prefix) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const globalVersion = fs_1.readFileSync(`${heroicToolsPath}/DXVK/latest_dxvk`)
            .toString()
            .split('\n')[0];
...
const showAboutWindow = () => {
    electron_1.app.setAboutPanelOptions({
        ...
        copyright: 'GPL V3',
        applicationVersion: `${electron_1.app.getVersion()} Absalom`,
