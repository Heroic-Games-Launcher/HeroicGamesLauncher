...
electronTest('webview', async (app) => {
  ...
  await test.step('goes back and forth inside webview and also to Heroic screens', async () => {
    ...
    // use loadURL (awaited until the page finishes loading) instead of setting
    // `src` directly and sleeping, which was flaky on slow networks
    await page.$eval('webview', (el: WebviewTag) => el.loadURL(GOOGLE_URL))
